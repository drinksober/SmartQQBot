...
def run_http_daemon(host="0.0.0.0", port=8888, http_only=False):
    import multiprocessing
    from smart_qq_bot.httpserver import run_server
    if http_only:
        run_server()
    p = multiprocessing.Process(
        target=run_server, kwargs={"host": host,
                                   "port": port})
    p.daemon = True
    p.start()


def main_loop(no_gui=False, new_user=False, debug=False, http=False, http_only=False):
    patch()
    if debug:
        logger.setLevel(logging.DEBUG)
    else:
        logger.setLevel(logging.INFO)
    if http or http_only:
        run_http_daemon(http_only=http_only)
